...
TEST_CASE("PARSING WORKS") {
    ...
    std::vector<Edge<std::string>> routes =
        readRoutesFromFile<std::string>("datasets/routes.txt", airports);
...
TEST_CASE("BUILDING GRAPH FROM PARSED DATA WORKS") {
    Graph<std::string> g =
        buildGraphFromFiles<std::string>("datasets/airports.txt", "datasets/routes.txt");
